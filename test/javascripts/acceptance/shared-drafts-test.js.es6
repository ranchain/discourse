...
QUnit.test('Viewing', assert => {
  ...
  andThen(() => {
    assert.ok(find('.shared-draft-notice').length === 1);
    let categoryChooser = selectKit('.shared-draft-notice .category-chooser');
  ...
  andThen(() => {
    assert.ok(find('.shared-draft-notice').length === 0);
  });
});
